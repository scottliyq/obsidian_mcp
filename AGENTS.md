...
你**必须**先执行以下检查/激活步骤之一，且**必须看到 (pyt312obsync)** 提示才可以继续：
你**必须**每次收到对话需求后，先回答**yes sir** 才继续任务,代码修改完成后更新当前目录的README.md文件，记录修改内容和修改原因,不要创建除了README.md以外的md文件记录改动。

```bash
conda activate py312obsync || echo "请先手动激活 py312obsync"
# 或者更保险：
source $(conda info --base)/etc/profile.d/conda.sh && conda activate py312obsync